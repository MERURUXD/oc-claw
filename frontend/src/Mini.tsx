    ...
    const unlisten = listen('claude-task-complete', (ev: any) => {
      ...
      if (ev.payload?.waiting && viewModeRef.current === 'efficiency' && autoExpandOnTaskRef.current) {
        ...
        if (sid) {
          ...
          const pi = ev.payload?.pendingInteraction
          setClaudeSessions((prev) => prev.map((s) => s.sessionId === sid ? {
            ...s,
            status: 'waiting',
            needsReview: isRev,
            ...(pi !== undefined ? { pendingInteraction: pi } : {}),
          } : s))
        }
                                          ...
                                          {(() => {
                                            ...
                                            const resolvePermission = (decision: string) => {
                                              ...
                                              if (cs.source === 'codex' && cs.pendingInteraction?.kind === 'approval') {
                                                ...
                                                const isActionableRelay = !!(cs.pendingInteraction.requestId && cs.pendingInteraction.turnId && cs.pendingInteraction.approvalActions?.canAllowTurn)
                                                ...
                                                // Detection-only mode: no live approval responder waiting yet.
                                                // Inform user to complete approval in Codex Desktop, with direct jump action.
                                                return (
                                                  <div className="flex flex-col gap-2 w-full">
                                                    <div className="text-[11px] text-amber-400/90 bg-amber-950/20 border border-amber-800/30 rounded px-2.5 py-1.5">
                                                      {t('mini.codexNativeRequired', '当前审批需要在 Codex 中完成')}
                                                    </div>
                                                    <div className="flex gap-2 w-full">
                                                      <button
                                                        data-no-drag
                                                        ...
                                                        className="flex-1 py-1.5 rounded-md text-[12px] font-medium bg-[#27272a] text-slate-200 hover:bg-[#323236] transition-colors border border-white/10"
                                                        title={t('mini.viewInCodexTooltip', '在 Codex Desktop 原生界面中处理')}
                                                      >
                                                        {t('mini.viewInCodex', '前往 Codex')}
                                                      </button>
                                                      <button
                                                        ...
                                                        className="px-2.5 py-1.5 rounded-md text-[11px] font-normal bg-[#202023] text-slate-400 hover:bg-[#27272a] hover:text-slate-300 transition-colors"
                                                      >
                                                        {t('mini.later', '稍后处理')}
                                                      </button>
                                                    </div>
