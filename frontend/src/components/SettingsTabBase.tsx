    ...
    <div className="max-w-2xl mx-auto pt-10 pb-20 px-6 flex flex-col gap-10">
      ...
      <section className="flex flex-col gap-4">
        ...
        <div className="bg-[#0f0f0f] border border-white/5 rounded-2xl overflow-hidden">
          ...
          <div className="flex items-center justify-between p-4 border-b border-white/5">
            <div className="flex flex-col gap-1">
              <span className="text-sm font-medium text-white/90">{t('settings.quotaRecoverySound', 'Quota Recovery Sound')}</span>
              <span className="text-xs text-white/40">{t('settings.quotaRecoverySoundDesc', 'Sound played when quota recovers to 10% or more')}</span>
            </div>
            <div className="flex bg-black/50 p-0.5 rounded-lg border border-white/5">
              {(['default'] as const).map((s) => (
                <button
                  key={s}
                  onClick={() => onChangeQuotaRecoverySound?.(s)}
                  className={`px-3 py-1 text-xs font-medium rounded-md transition-colors ${quotaRecoverySound === s ? 'bg-white/10 text-white' : 'text-white/40 hover:text-white/60'}`}
                >
                  {s === 'default' ? t('settings.defaultSound', 'Default') : s}
                </button>
              ))}
            </div>
          </div>
