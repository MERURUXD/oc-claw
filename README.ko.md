...
<p align="center">
  ...
  <sub>Windows: OpenClaw, Claude Code, Cursor, Codex, OpenCode, Gemini CLI, Google Antigravity, Hermes Agent (원격 SSH)</sub>
</p>
<p align="center">
...
- OpenClaw / Claude Code / Codex / Cursor / Gemini CLI / Google Antigravity / Hermes Agent 에이전트 활동에 실시간 반응 (작업 중, 유휴, 대기)
...
- Hook을 통해 로컬 Claude Code, Codex, Cursor, Gemini CLI, Google Antigravity 세션을 수신하고 실시간 대화 표시
...
- macOS 또는 Windows
- [OpenClaw](https://github.com/nicepkg/openclaw), [Claude Code](https://docs.anthropic.com/en/docs/claude-code), [Codex](https://github.com/openai/codex), [Cursor](https://www.cursor.com), [Gemini CLI](https://github.com/google-gemini/gemini-cli), [Google Antigravity](https://antigravity.google), [Hermes Agent](https://github.com/NousResearch/hermes-agent) 중 하나 이상 설치 필요
...
Google Antigravity ──→ Hooks ──→ 이벤트 파서 ──→ 활동 상태
Hermes Agent       ──→ Plugin ──→ 이벤트 파서 ──→ 활동 상태
                                                   ↓
                 애니메이션 스프라이트 ← 상태 머신 ← 사운드 효과
```

OC-Claw는 OpenClaw 세션 파일을 폴링하여 에이전트 활동을 감지하고, 설치된 Hook/플러그인을 통해 Claude Code, Codex, Cursor, Gemini CLI, Google Antigravity, Hermes Agent를 수신합니다. 활동 상태가 노치 섬(macOS) 또는 작업 표시줄 영역(Windows)의 캐릭터 애니메이션을 구동하며, 확장 패널에서 세션 상세, 채팅 기록, 메트릭을 확인할 수 있습니다.
