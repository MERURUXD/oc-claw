...
This material was moved from the root CLAUDE.md at commit `19282b5733cad757455256578ad094e6053e0b0e`. It records earlier investigations, not a verified description of the current application or active agent instructions. Some obsolete lifecycle/polling advice and package-manager examples were corrected during migration.

Follow [AGENTS.md](../../AGENTS.md) for current working rules. Consult only the section relevant to a concrete investigation, and verify implementation details against current code and tests. Old paths, UI dimensions, hook payloads, commands, and platform assumptions below are historical examples, not mandatory setup or steps to execute. Release procedures are not authorization to publish; current checked-in workflows define release behavior.
...
1. **Verify data formats for the affected integration.** Prefer checked-in parsers and sanitized fixtures. The original investigation used `~/.openclaw/agents/*/sessions/*.jsonl`; that machine-specific path is not a required prerequisite.
...
6. **OpenClaw source code** is installed at `~/Library/pnpm/global/5/.pnpm/openclaw@*/node_modules/openclaw/`. Check `dist/session-key-*.js` for session key format, `dist/health-*.js` for health endpoint logic. These are historical machine-specific locations, not required setup. Use current source and sanitized fixtures for the task; request a sample only if missing evidence blocks it.
...
2. **PID-alive check must cover ALL active statuses**, not just "waiting". If CC is killed (Ctrl+C / SIGKILL) during thinking (no tool calls yet), no Stop/SessionEnd hook fires. Without the PID check on "processing"/"tool_running"/"compacting", the character gets stuck in working state forever. Timeouts may trigger a liveness investigation, but elapsed time alone must not complete a live Codex/Cursor turn. Verify lifecycle evidence and required descendants before changing status.
...
1. **Avoid starving slow polling requests while isolating obsolete results.** If every interval supersedes an unfinished request, slow SSH results may never be applied. Bound concurrent work and distinguish request age from ownership: results belonging to an old configuration, session, or turn must not overwrite the current owner's state. A settings-mode pause alone does not establish ownership after settings reopen or change.
...
2. **Build platform installers**:
   - **Windows**: `pnpm exec tauri build` in `frontend/`. Output: `frontend/src-tauri/target/release/bundle/nsis/oc-claw_<ver>_x64-setup.exe` (~35 MB).
   - **macOS**: `pnpm exec tauri build` on a Mac. Output: `frontend/src-tauri/target/release/bundle/dmg/oc-claw_<ver>_aarch64.dmg`. Requires signing & notarization for public distribution.
...
- **Fullscreen detection**: A background thread polls every 500ms using `MonitorFromWindow` + `GetMonitorInfoW` to check if the foreground window covers its entire monitor AND is not a normal maximized window (a maximized window also covers the monitor but must NOT trigger the hide). `fullscreen_foreground_monitor()` excludes desktop shell windows (Progman/WorkerW/Shell_TrayWnd) and oc-claw's own process (via `GetWindowThreadProcessId`), then distinguishes maximized from true fullscreen with `IsZoomed` + `GetWindowLongPtrW(GWL_STYLE)`: maximized windows carrying `WS_CAPTION`/`WS_THICKFRAME` decorations do not trigger the hide; classic fullscreen (`IsZoomed == false`, e.g. F11/PowerPoint/exclusive games) and undecorated borderless fullscreen (`IsZoomed == true` without those styles) do. When detected:
...
- `pnpm exec tauri build` produces an NSIS installer `.exe` in `src-tauri/target/release/bundle/nsis/` and a standalone binary in `src-tauri/target/release/ooclaw.exe`.
- Windows builds do not require code signing (users may see SmartScreen warnings).
- When `pnpm exec tauri dev` fails with "拒绝访问 (os error 5)", it means the old `ooclaw.exe` process is still running and must be killed before recompilation can replace the binary.
